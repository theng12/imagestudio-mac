...
class GenerationManager:
    ...
    def delete_job(self, job_id: str) -> bool:
        """Remove one finished job from history AND delete its PNG file from disk.
    ...
    def output_stats(self) -> dict:
        """Total size + count of generated PNGs in the outputs folder — so the UI
    ...
    def prune_outputs(self, keep_last: int = 0, older_than_days: float = 0.0) -> dict:
        """Delete PNG files to reclaim disk. Exactly one mode:
        ...
        if keep_last > 0:
            to_delete = pngs[keep_last:]
        elif older_than_days > 0:
            cutoff = time.time() - older_than_days * 86400
            to_delete = [p for p in pngs if p.stat().st_mtime < cutoff]
        else:
            return {"deleted": 0, "freed_bytes": 0}
        freed = 0
        deleted = 0
