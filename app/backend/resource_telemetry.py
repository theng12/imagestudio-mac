"""Best-effort resource evidence for one local image generation job.

Image Studio owns this measurement because it owns the model process. The
schema deliberately matches Voice Studio so Studio Hub qualification tooling
can compare local workloads across the same 8/16/24 GB fleet tiers.
"""
...
class JobResourceSampler:
    ...
    def start(self) -> "JobResourceSampler":
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="image-job-resource-sampler",
            daemon=True,
        )
        self._thread.start()
        return self
    ...
    def _take(self, *, reset_mlx_peak: bool = False) -> None:
        ...
        with self._lock:
            ...
            self._min_available = min(
                host["available_gb"],
                self._min_available
                if self._min_available is not None
                else host["available_gb"],
            )
            self._max_host_used = max(
                host["used_gb"],
                self._max_host_used
                if self._max_host_used is not None
                else host["used_gb"],
            )
            self._max_host_percent = max(
                host["used_percent"],
                self._max_host_percent
                if self._max_host_percent is not None
                else host["used_percent"],
            )
            self._max_swap = max(
                host["swap_used_gb"],
                self._max_swap
                if self._max_swap is not None
                else host["swap_used_gb"],
            )
            self._peak_worker_rss = max(
                worker["rss_gb"],
                self._peak_worker_rss
                if self._peak_worker_rss is not None
                else worker["rss_gb"],
            )
            self._peak_process_count = max(
                self._peak_process_count, worker["process_count"]
            )
            if mlx["supported"]:
                for attr, key in (
    ...
    def _summary_locked(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mlx": {
                "supported": bool(
                    start_mlx.get("supported") or end_mlx.get("supported")
                ),
